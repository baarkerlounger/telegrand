...
use crate::session::content::message_row::MessageMediaContent;
...
mod imp {
    ...
    pub(crate) struct MessageMedia {
        ...
        #[template_child]
        pub(super) content: TemplateChild<MessageMediaContent>,
    }
...
glib::wrapper! {
    pub(crate) struct MessageMedia(ObjectSubclass<imp::MessageMedia>)
...
impl MessageMedia {
    ...
    fn load_media_from_path(&self, path: &str) {
        ...
        let texture = gdk::Texture::from_filename(path).unwrap().upcast();
        self.imp().content.set_paintable(Some(texture));
    }
}
